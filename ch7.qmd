...
となり，$\beta_2\mathrm{Cov}({X_1X_2})/\mathrm{Var}({X_1})$分のバイアスがかかります。このバイアスは，$\mathrm{Cov}({X_1X_2})=0$でない限り存在します[^ch7-1]。これは，疑似相関の際に起こるバイアスを表しています。

[^ch7-1]: 推定するモデル @eq-7-7 に，正しいモデルを代入すると，
